...
The first four questions target real operational ambiguity discovered in the dataset and the applied rules engine. The last five address the production delivery boundary, where this prototype's inputs and outputs actually need to plug into Apex's existing systems, timelines, and governance requirements.
...
### 5. What are the upstream ingest source and downstream audit logging destinations?
...
### 6. Does this pipeline need to run on a schedule, react to file drops, or poll an API?
Separate from where the data comes from is the question of how ingestion actually gets triggered. Does this need to run as an automated daily job once your overnight systems settle, or does it need to initiate as files land throughout the day, or monitor an API for changes? This determines whether we are building a scheduled batch job or an event driven service, which is a meaningfully different architecture.

### 7. What regulatory framework should the audit trail architecture be built around?
...
### 8. How time sensitive are escalated trades, and does triage need to block settlement?
...
### 9. Where do post review actions happen, and what are the access control requirements?
