...
This system automates the *triage*. It automatically clears `L1` standard bounds (`<= $500k` Notional, no flags) and routes `L2`, `L3`, and `L4` escalations to humans. It calculates specific derived fields (like `> 3%` price variance and `> 24hr` booking delays).

> It does **not** employ AI to make subjective compliance judgments, by design.
